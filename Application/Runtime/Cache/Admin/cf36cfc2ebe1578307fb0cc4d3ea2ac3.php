	...
	<form class="form form-horizontal" id="form-admin-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($data["role_name"]); ?>" placeholder="" id="role_name" name="role_name">
				<input type="hidden" class="input-text" value="<?php echo ($data["role_id"]); ?>" placeholder="" id="role_id" >
			</div>
		</div>
		<div class="row cl">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><dl class="permission-list">
					<dt>
						<label>
							...
							<?php if(is_array($role_auth)): $i = 0; $__LIST__ = $role_auth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$moo): $mod = ($i % 2 );++$i; if(($moo["auth_id"]) == $foo["auth_id"]): ?>checked<?php endif; endforeach; endif; else: echo "" ;endif; ?>
							>
					...
					<?php if(is_array($foo['son'])): $i = 0; $__LIST__ = $foo['son'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><dl class="cl permission-list2">
							<dt>
								<label class="">
									...
									<?php if(is_array($role_auth)): $i = 0; $__LIST__ = $role_auth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$moo): $mod = ($i % 2 );++$i; if(($moo["auth_id"]) == $voo["auth_id"]): ?>checked<?php endif; endforeach; endif; else: echo "" ;endif; ?>
									>
							...
							<?php if(is_array($auth)): $i = 0; $__LIST__ = $auth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><dd>
								<?php if(($vo["auth_pid"]) == $voo["auth_id"]): ?><label class="">
									&nbsp;
											...
											<?php if(in_array(($vo["auth_id"]), is_array($imps)?$imps:explode(',',$imps))): ?>checked<?php endif; ?>
											>
...
$('.btn-success').click(function(msg){
	...
	// 将获取到的数据提交到后台
	$.post("<?php echo U('Admin/Role/edit');?>",{role_name:role_name,ids:ids,role_id:role_id},function(data){
		if(data.code==1){
			// 数据修改成功
