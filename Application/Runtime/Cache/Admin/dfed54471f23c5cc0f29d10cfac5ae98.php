...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="<?php echo ($data["category_name"]); ?>" placeholder="" id="category_name" >
			<input type="hidden" class="input-text" value="<?php echo ($data["category_id"]); ?>" placeholder="" id="category_id" >
		</div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			...
			<option value="0" 
			<?php if(($data["pid"]) == "0"): ?>selected<?php endif; ?>
			>无父类</option>
				<?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($foo["category_id"]); ?>"
					<?php if(($data["pid"]) == $foo["category_id"]): ?>selected<?php endif; ?>
...
$('.btn-primary').click(function(msg){
	...
	var category_id=$('#category_id').val();	// 分类ID
	var category_name=$('#category_name').val();	// 分类名称
	var pid=$('#pid').val();	// 父类ID

	//判断提交的内容是否为空
	if(category_name=='' || category_name==null || category_name==undefined){
		layer.alert('分类名称不可为空');
	}

	//将内容发送到PHP
	// $.post{发送的地址，内容，回调}
	$.post("<?php echo U('Admin/Category/edit');?>",{category_id:category_id,category_name:category_name,pid:pid},function(data){
			if(data.code==1){
				// 数据修改成功
