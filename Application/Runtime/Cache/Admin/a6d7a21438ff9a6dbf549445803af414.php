...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="<?php echo ($data["speci_name"]); ?>" placeholder="" id="speci_name" >
			<input type="hidden" class="input-text" value="<?php echo ($data["speci_id"]); ?>" placeholder="" id="speci_id" >
		</div>
	</div>
	<div class="row cl">
...
$('.btn-primary').click(function(msg){
	...
	var speci_id=$('#speci_id').val();		// 规格ID
	var speci_name=$('#speci_name').val();	// 规格名称

	//将内容发送到PHP
	$.post("<?php echo U('Admin/Speci/edit');?>",{speci_id:speci_id,speci_name:speci_name},function(data){
			if(data.code==1){
				// 数据修改成功
