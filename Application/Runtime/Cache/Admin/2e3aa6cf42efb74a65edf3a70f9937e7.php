...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>选择题<span class="c-gray en">&gt;</span> 选择题管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
	<a href="javascript:;" onclick="append('添加课堂','<?php echo U('Admin/Choice/append');?>','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加选择题</a></span> 
</div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					<td><?php echo ($foo["choice_content"]); ?></td>
					<td><?php echo ($foo["choice_a"]); ?></td>
					<td><?php echo ($foo["choice_b"]); ?></td>
					<td><?php echo ($foo["choice_c"]); ?></td>
					<td><?php echo ($foo["choice_d"]); ?></td>
					<td>
						<?php switch($foo["choice_true"]): case "0": ?>A<?php break;?>
	                        <?php case "1": ?>B<?php break;?>
	                        <?php case "2": ?>C<?php break;?>
	                        <?php case "3": ?>D<?php break;?>
	                        <?php default: endswitch;?>
                    </td>
					<td><?php echo (date("Y-m-d H:i:s",$foo['choice_time'])); ?></td>
					<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="edit('内容编辑','<?php echo U('Admin/Choice/edit');?>?id=<?php echo ($foo["choice_id"]); ?>',<?php echo ($foo["choice_id"]); ?>,'800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
					<a title="删除" href="javascript:;" onclick="del(this,<?php echo ($foo["choice_id"]); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: "<?php echo U('Admin/Choice/del');?>",
			dataType: 'json',
			data:'id='+id,
