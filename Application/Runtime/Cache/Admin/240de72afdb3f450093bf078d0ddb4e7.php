...
<![endif]-->
<title>添加产区 - 产区管理</title>
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" id="form-admin-add" >
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>产区名称：</label>
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="<?php echo ($data["place_name"]); ?>" placeholder="" id="place_name" >
			<input type="hidden" class="input-text" value="<?php echo ($data["place_id"]); ?>" placeholder="" id="place_id" >
		</div>
	</div>
	<div class="row cl">
...
$('.btn-primary').click(function(msg){
	...
	var place_id=$('#place_id').val();		// 产区ID
	var place_name=$('#place_name').val();	// 产区名称

	//将内容发送到PHP
	$.post("<?php echo U('Admin/Place/edit');?>",{place_id:place_id,place_name:place_name},function(data){
			if(data.code==1){
				// 数据修改成功
