...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<textarea style="width: 300px; height: 130px;" type="text" class="input-text" id="mission" name="mission"><?php echo ($data["mission"]); ?></textarea> 
		</div>
	</div>

	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">针对学习哪一个厅：</label>
		<div class="formControls col-xs-8 col-sm-3">
			<label>
				<input type="checkbox" style="width:15%;
		height: 25px;"  class="input-text" id="xxting" checked="<?php if(($xxting) == "1"): ?>true<?php endif; ?>" >
				厅1
			</label>
			<label>
				<input  type="checkbox" style="width:15%;
		height: 25px;" class="input-text" id="xxting2" checked="<?php if(($xxting2) == "1"): ?>true<?php endif; ?>">
				厅2
			</label>
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-6">
			...
			<div>
				<h3 id='h3Ele'><?php echo (date('Y-m-d',$data["schedule"])); ?></h3>
			</div>
		</div>
	</div>
...
submit.onclick=function(event){
	...
	if(mission==''){
		alert('请填写学习任务');return false;
	}

	var xxting = '';
	if($('#xxting').prop("checked")==true){
		xxting = '1,';
	}
	if($('#xxting2').prop("checked")==true){
		xxting = xxting+'2,';
	}

	if(xxting2=='0'&&xxting=='0'){
	...
	$.ajax({
		...
		// dataType:'json',
		// 提交地址
		url:"<?php echo U('Admin/user/xxedit');?>",
		// async:true：异步提交
		async:true,
		data:fd,
