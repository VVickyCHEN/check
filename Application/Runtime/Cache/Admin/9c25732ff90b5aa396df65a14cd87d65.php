	...
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					<td><?php echo ($foo["banner_id"]); ?></td>
					<td><img src="/gt/Public/<?php echo (ltrim($foo["banner_thumb"],'./Public')); ?>"></td>
					<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="edit('轮播图编辑','<?php echo U('Admin/Banner/edit');?>?id=<?php echo ($foo["banner_id"]); ?>',<?php echo ($foo["banner_id"]); ?>,'800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
					</td>
