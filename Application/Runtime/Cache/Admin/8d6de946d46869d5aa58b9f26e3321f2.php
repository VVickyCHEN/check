...
<![endif]-->
<title>修改款式 - 款式管理</title>
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" id="form-admin-add" >
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>款式名称：</label>
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="<?php echo ($data["style_name"]); ?>" placeholder="" id="style_name" >
			<input type="hidden" class="input-text" value="<?php echo ($data["style_id"]); ?>" placeholder="" id="style_id" >
		</div>
	</div>
	<div class="row cl">
...
$('.btn-primary').click(function(msg){
	...
	var style_id=$('#style_id').val();	// 款式ID
	var style_name=$('#style_name').val();	// 款式名称

	//判断提交的内容是否为空
	if(style_name=='' || style_name==null || style_name==undefined){
		layer.alert('款式名称不可为空');
	}

	//将内容发送到PHP
	// $.post{发送的地址，内容，回调}
	$.post("<?php echo U('Admin/style/edit');?>",{style_id:style_id,style_name:style_name},function(data){
			if(data.code==1){
				// 数据修改成功
