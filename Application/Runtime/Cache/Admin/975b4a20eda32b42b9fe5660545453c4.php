...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="" placeholder="" id="product_name" name="product_name" >
		</div>
	</div>
	<div class="row cl">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="product_place" id="product_place" size="1">
				<?php if(is_array($place)): $i = 0; $__LIST__ = $place;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($foo["place_id"]); ?>"><?php echo ($foo["place_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			</span> </div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="product_region" id="product_region" size="1">
				<?php if(is_array($region)): $i = 0; $__LIST__ = $region;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($foo["region_id"]); ?>"><?php echo ($foo["region_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			</span> </div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="product_speci" id="product_speci" size="1">
				<?php if(is_array($speci)): $i = 0; $__LIST__ = $speci;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($foo["speci_id"]); ?>"><?php echo ($foo["speci_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			</span> </div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<input type="text" class="input-text" value="" placeholder="" id="product_min" name="product_min" >
		</div>
	</div>
	<div class="row cl">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<script id="product_detail" type="text/plain" name="product_detail" style="width:500px;height:200px;"></script>
		</div>
	</div>
	<div class="row cl">
...
submit.onclick=function(event){
	...
	var fd = new FormData();
	// 添加数据到fd
	fd.append("product_name",$('#product_name').val());			//商品名称
	fd.append("product_min",$('#product_min').val()); 			//商品最低价
	...
	fd.append("product_price",$('#product_price').val());		//商品价格
	fd.append("product_num",$('#product_num').val());			//商品库存
	fd.append("product_detail",ue.getContent());				//商品水印
	...
	$.ajax({
		...
		type:"post",
		// 提交地址
		url:"<?php echo U('Admin/Product/append');?>",
		// async:true：异步提交
		async:true,
		data:fd,
