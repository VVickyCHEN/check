...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-3">
			<input type="hidden" name="judge_id" id="judge_id" value="<?php echo ($data["judge_id"]); ?>">
			<textarea text style="width: 300px;height: 100px;" id="judge_content" name="judge_content"><?php echo ($data["judge_content"]); ?></textarea>
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="judge_true" id="judge_true" size="1">
				<?php if(($foo["judge_true"]) == "0"): ?><option value="0" selected>正确</option>
					<option value="1">错误</option>
				<?php else: ?>
					<option value="0">正确</option>
					<option value="1" selected>错误</option><?php endif; ?>
			</select>
			</span> 
		</div>
	</div>
...
$('#submit').click(function(msg){
	...
	//将内容发送到PHP
	$.post("<?php echo U('Admin/Judge/edit');?>",{judge_id:judge_id,judge_content:judge_content,judge_true:judge_true},function(data){
			if(data.code==1){
				// 数据修改成功
