...
<div class="page-container">
	...
	<a href="javascript:;" onclick="append('添加检修','<?php echo U('Admin/program/append');?>','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加检修</a></span> 
</div>
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					<td><?php echo ($foo["title"]); ?></td>
					<td>
						<img style="width: 150px;height: 150px;" src="/check/Public/<?php echo (ltrim($foo["img"],'./Public')); ?>">
					</td>
					<td><?php echo ($foo["standard"]); ?></td>
					<td><?php echo ($foo["type"]); ?></td>
					<td><?php echo ($foo["updated_at"]); ?></td>
					<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="edit('内容编辑','<?php echo U('Admin/program/edit');?>?id=<?php echo ($foo["id"]); ?>',<?php echo ($foo["id"]); ?>,'800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: "<?php echo U('Admin/program/del');?>",
			dataType: 'json',
			data:'id='+id,
