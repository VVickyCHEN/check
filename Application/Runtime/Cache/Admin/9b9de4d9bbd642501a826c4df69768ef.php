...
<![endif]-->
<title>修改轮播图 - 轮播图管理</title>
</head>
<body>
<article class="page-container">
	<form class="form form-horizontal" method="post" id="form-admin-add" enctype="multipart/form-data" >
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>添加轮播图：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="file" id="imgfile" multiple name="img[]">
			<input type="hidden" class="input-text" value="<?php echo ($data["banner_id"]); ?>" placeholder="" id="banner_id" >
		</div>
	</div>
	<div class="row cl">
...
submit.onclick=function(event){
	...
	var fd = new FormData();
	// 添加数据到fd
	fd.append("banner_id",$('#banner_id').val());				//轮播图ID
	fd.append("imgfile",imgfile.files[0]);						//轮播图
	fd.append("waterfile",waterfile.files[0]);					//轮播图水印
	...
	$.ajax({
		...
		// dataType:'json',
		// 提交地址
		url:"<?php echo U('Admin/Banner/edit');?>",
		// async:true：异步提交
		async:true,
		data:fd,
