	...
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			<?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					<td style="text-align:left"><?php echo ($foo["auth_name"]); ?></td>
					<td><?php echo ($foo["auth_module"]); ?></td>
					<td><?php echo ($foo["auth_controller"]); ?></td>
					<td><?php echo ($foo["auth_action"]); ?></td>
					<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="edit('权限编辑','<?php echo U('Admin/Auth/edit');?>?id=<?php echo ($foo["auth_id"]); ?>',<?php echo ($foo["auth_id"]); ?>,'800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
					<a title="删除" href="javascript:;" onclick="del(this,<?php echo ($foo["auth_id"]); ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
