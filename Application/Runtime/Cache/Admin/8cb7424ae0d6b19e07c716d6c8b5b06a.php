	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<select id="auth_pid">
				...
				<?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$foo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($foo["auth_id"]); ?>"><?php echo ($foo["auth_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
		</div>
	</div>
	<div class="row cl">
...
// alert(auth_name+'-'+auth_pid+'-'+auth_module+'-'+auth_controller+'-'+auth_action+'-'+auth_type);
	...
	// 将数据用ajax方法提到后台
	$.post("<?php echo U('Admin/Auth/append');?>",{auth_name:auth_name,auth_pid:auth_pid,auth_module:auth_module,auth_controller:auth_controller,auth_action:auth_action,auth_type:auth_type},function(data){
		if(data.code==1){
			// 数据添加成功
